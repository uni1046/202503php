...
@section('content')
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-6">{{ __('Create New Post') }}</h1>
    ...
    <form action="{{ route('posts.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}" required
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('title') @enderror">
            @error('title')
        ...
        <div>
            ...
            <textarea name="content" id="content" rows="10" required
                      class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('content') @enderror">{{ old('content') }}</textarea>
            @error('content')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="author_id" id="author_id" required
                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-700 dark:text-gray-200 @error('author_id') @enderror">
        ...
        <div>
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('slug') @enderror">
            @error('slug')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="status" id="status" required
                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-700 dark:text-gray-200 @error('status') @enderror">
                <option value="1" {{ old('status', 1) == 1 ? 'selected' : '' }}>{{ __('Published') }}</option>
                <option value="0" {{ old('status') == 0 ? 'selected' : '' }}>{{ __('Draft') }}</option>
                <option value="2" {{ old('status') == 2 ? 'selected' : '' }}>{{ __('Hidden') }}</option>
            </select>
            @error('status')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        ...
        {{-- If you want to add Tag selection here, you'll need to pass $tags to this view
             and implement a multi-select input. For example:
        <div>
