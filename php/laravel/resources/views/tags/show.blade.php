...
@section('title', __('Tag') . ': ' . $tag->name)

@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $tag->name }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $tag->slug }}</p>
        </div>
        <div class="space-x-2">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 space-y-4">
        @if($tag->description)
            <div>
                ...
                <p class="mt-1 text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap">{{ $tag->description }}</p>
            </div>
        @endif
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Created') }}</h3>
            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $tag->created_at->translatedFormat('F j, Y') }} ({{ $tag->created_at->diffForHumans() }})</p>
        </div>
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Last Updated') }}</h3>
            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $tag->updated_at->translatedFormat('F j, Y') }} ({{ $tag->updated_at->diffForHumans() }})</p>
        </div>
    </div>

    <div class="mt-8">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">{{ __('Posts with this Tag') }} ({{ $tag->posts->count() }})</h2>
        @if($tag->posts->count() > 0)
                        ...
                        <div class="p-6">
                            ...
                            @if($post->author)
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    {{ __('By') }} <a href="{{ route('authors.show', $post->author) }}" class="hover:underline">{{ $post->author->name }}</a>
                                </p>
                            @endif
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ Str::limit($post->content) }}</p>
            ...
            @if ($tag->posts instanceof \Illuminate\Pagination\LengthAwarePaginator && $tag->posts->hasPages())
                <div class="mt-6">
                    {{ $tag->posts->links() }}
                </div>
            @endif
        @else
            <p class="text-gray-500 dark:text-gray-400">{{ __('No posts are currently associated with this tag.') }}</p>
        @endif
    </div>
@endsection
