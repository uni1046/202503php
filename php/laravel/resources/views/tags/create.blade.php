...
@section('content')
    <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-6">{{ __('Create New Tag') }}</h1>
    ...
    <form action="{{ route('tags.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" required
                   class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('name') @enderror">
            @error('name')
        ...
        <div>
            ...
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400" id="slug-description">
                {{ __('The slug is usually auto-generated from the name if left empty, or you can provide a custom one. It should be unique and URL-friendly (e.g., "my-awesome-tag").') }}
            </p>
            @error('slug')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('description') @enderror">{{ old('description') }}</textarea>
            @error('description')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
